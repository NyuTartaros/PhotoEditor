...
class Bucket extends Drawing{
	BufferedImage image;
	public Bucket(BufferedImage image) {
	...
	void draw(Graphics2D g2d){
		...
		FloodFillAlgorithm ffa = new FloodFillAlgorithm(image);
		ffa.floodFillScanLineWithStack(x1, y1, new Color(R, G, B).getRGB(), ffa.getColor(x1, y1));
		ffa.updateResult();  
		image = ffa.getInputImage();
